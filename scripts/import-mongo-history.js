...
async function prepareUserMerge(db, pg, collectionName, cityIdMap) {
  ...
  const buildRow = (d, id) => {
    ...
    return {
      ...
      menuFlowEnabled: toBool(d.menuFlowEnabled, false),
      menuFlowCompanyId: nonBlank(d.menuFlowCompanyId),
...
async function main() {
  ...
  try {
    ...
    const userColumns = [
      ...
      'ifoodOrdersUsed','ifoodOrdersAvailable','menuFlowEnabled','menuFlowCompanyId','createdAt','createdBy','updatedAt',
    ];
    ...
    results.deliveries = await importCursor({
      ...
      columns: [
        ...
        'ifoodLastEventCode','ifoodLastEventFullCode','ifoodConfirmedAt','releasedAt','releasedBy','source','menuFlowOrderId',
        'menuFlowOrderNumber','menuFlowCompanyId','menuFlowRestaurantName','menuFlowSubtotalCents','menuFlowDeliveryFeeCents',
        'menuFlowServiceFeeCents','menuFlowDiscountCents','menuFlowTotalCents','menuFlowPaymentMethod','menuFlowNeedsChange',
        'menuFlowChangeForCents','menuFlowExpectedChangeCents','menuFlowItems','menuFlowImportedAt','menuFlowSyncPending',
        'menuFlowLastSyncAt','menuFlowLastSyncedStatus','menuFlowSyncError','arrivedAtDestinationAt',
      ...
      mapper: (d) => {
        ...
        return {
          ...
          source: d.source ?? (d.menuFlowOrderId ? 'MENU_FLOW' : null),
          menuFlowOrderId: d.menuFlowOrderId ?? null,
          menuFlowOrderNumber: d.menuFlowOrderNumber ?? null,
          menuFlowCompanyId: d.menuFlowCompanyId ?? null,
          menuFlowRestaurantName: d.menuFlowRestaurantName ?? null,
          menuFlowSubtotalCents: d.menuFlowSubtotalCents == null ? null : toBigIntString(d.menuFlowSubtotalCents, '0'),
          menuFlowDeliveryFeeCents: d.menuFlowDeliveryFeeCents == null ? null : toBigIntString(d.menuFlowDeliveryFeeCents, '0'),
          menuFlowServiceFeeCents: d.menuFlowServiceFeeCents == null ? null : toBigIntString(d.menuFlowServiceFeeCents, '0'),
          menuFlowDiscountCents: d.menuFlowDiscountCents == null ? null : toBigIntString(d.menuFlowDiscountCents, '0'),
          menuFlowTotalCents: d.menuFlowTotalCents == null ? null : toBigIntString(d.menuFlowTotalCents, '0'),
          menuFlowPaymentMethod: d.menuFlowPaymentMethod ?? null,
          menuFlowNeedsChange: d.menuFlowNeedsChange == null ? null : toBool(d.menuFlowNeedsChange, false),
          menuFlowChangeForCents: d.menuFlowChangeForCents == null ? null : toBigIntString(d.menuFlowChangeForCents, '0'),
          menuFlowExpectedChangeCents: d.menuFlowExpectedChangeCents == null ? null : toBigIntString(d.menuFlowExpectedChangeCents, '0'),
          menuFlowItems: cleanJson(Array.isArray(d.menuFlowItems) ? d.menuFlowItems : []),
          menuFlowImportedAt: toDate(d.menuFlowImportedAt),
          menuFlowSyncPending: toBool(d.menuFlowSyncPending, false),
          menuFlowLastSyncAt: toDate(d.menuFlowLastSyncAt),
          menuFlowLastSyncedStatus: d.menuFlowLastSyncedStatus ?? null,
          menuFlowSyncError: d.menuFlowSyncError ?? null,
